...
SSTT is an OCaml library for manipulating set-theoretic types ([documentation](anonymized)).  
...
It also provides a REPL ([web version](anonymized)) that allows performing common operations (subtyping, tallying, etc.) with a conveninent syntax.
...
```
opam pin sstt https://anonymized
```

The REPL binary `sstt` can also be installed this way:

```
opam pin sstt-repl https://anonymized
opam pin sstt-bin https://anonymized
```
