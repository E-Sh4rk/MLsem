...
then
    dune exec -- src/bin/native.exe -notime tests/*.ml  > "$REF_FILE"
    exit 0
elif [ "$1" = "diff" ]
then
     dune exec --display=quiet -- src/bin/native.exe -notime tests/*.ml > "$TMP_DIR"/types.tmp
...
then
    ...
    do
        echo -n Run "$i ... "
        dune exec --display=quiet -- src/bin/native.exe tests/*.ml > "$TMP_DIR"/timing.tmp
